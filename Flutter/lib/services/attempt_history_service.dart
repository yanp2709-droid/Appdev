...
class AttemptHistoryService {
  ...
  Future<List<AttemptHistoryModel>> getHistory({int page = 1, int perPage = 15}) async {
    try {
  ...
  Future<AttemptDetailModel> getAttemptDetail({required int attemptId}) async {
    try {
      final response = await apiClient.dio.get('/quiz/attempts/$attemptId/detail');
      final data = response.data as Map<String, dynamic>;
      final detail = AttemptDetailModel.fromJson(data['data'] as Map<String, dynamic>);
      return detail;
    } on DioException catch (e) {
      throw apiClient.handleException(e);
    }
